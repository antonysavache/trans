...
export class TronApiFixedService {
  ...
  private async fetchTRC20Transactions(address: string, startTimestamp?: number): Promise<Transaction[]> {
    try {
      ...
      if (!response.data.data || !Array.isArray(response.data.data)) {
        this.logger.warn(`No TRC20 transaction data found for address ${address}`);
        return [];
      }
      ...
      this.logger.error(`Error fetching TRC20 transactions for address ${address}`, error);
      this.logger.log('Using mock TRC20 transaction data instead');
  ...
  private async fetchTRXTransactions(address: string, startTimestamp?: number): Promise<Transaction[]> {
    try {
      ...
      if (!response.data.data || !Array.isArray(response.data.data)) {
        this.logger.warn(`No TRX transaction data found for address ${address}`);
        return [];
      }
      ...
      this.logger.error(`Error fetching TRX transactions for address ${address}`, error);
      this.logger.log('Using mock TRX transaction data instead');
